...
class InvestmentParamsOverride(BaseModel):
    ...
    agent_commission_months: float | None = Field(
        ...
        description="Broker fee in months of rent paid once per tenant turnover. Default 0.5.",
    )
    ...
    maintenance_pct: float | None = Field(
        ...
        description="Annual maintenance/running costs as a fraction of gross rent. Default 5%.",
    )
    property_tax_pct: float | None = Field(
        default=0.003, ge=0.0, le=0.05,
        description="Annual property tax as a fraction of sale price. Default 0.3%.",
    )
    inflation_rate_pct: float | None = Field(
        default=12.3, ge=0.0, le=100.0,
        description="Expected annual CPI inflation (%). Default 12.3% — KZ 2025 actual.",
    )
    risk_premium_pct: float | None = Field(
        default=3.0, ge=0.0, le=20.0,
        description="Extra return above inflation required for illiquid real estate (pp). Default 3 pp.",
    )
...
class InvestmentResponse(BaseModel):
    ...
    # Sensitivity scenarios — same params, rent value swapped for the 90% conformal
    # interval bounds. Help the user see how variance in achievable rent shifts the
    # verdict without changing any economic assumption.
...
def evaluate_investment(
    ...
    # different achievable rent. Lets the user see how realistic rent variance shifts
